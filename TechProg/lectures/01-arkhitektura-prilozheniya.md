...
проверять. Фаулер называет такой стиль Transaction Script — в русском издании книги Хононова
**транзакционный сценарий**, — и это не ругательство:
...
|---|---|---|---|
| **Domain** | сущности, объекты-значения, агрегаты, события предметной области, доменные исключения | ни на что (только BCL) | изменились **правила бизнеса** |
...
Здесь прячется неприятность, к которой мы вернёмся уже в следующей лекции. Правило
иногда требует данных, которых у объекта нет: чтобы проверить `BR-02`, нужно знать,
есть ли у этого человека другая активная кандидатура, а это вопрос к хранилищу.
Совместить полноту правил в домене, независимость домена от инфраструктуры
и отсутствие лишних обращений наружу одновременно не получается — приходится чем-то
жертвовать. У этого выбора есть имя: трилемма DDD, по Владимиру Хорикову.
...
| 7 | События | события предметной области, outbox, уведомления |
...
| **Транзакционный сценарий** (Transaction Script) | Стиль: сценарий целиком в одной процедуре (Фаулер) | код из блока 2 |
