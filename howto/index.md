...
Имя папки и ветки складывается из курса, вида работы и номера:

| Вид работы | Папка | Ветка |
|---|---|---|
| Лабораторная | `OOP/lab-01/` | `OOP/lab-01` |
| Домашнее задание | `OOP/hw-01/` | `OOP/hw-01` |
| Самостоятельная | `OOP/self-01/` | `OOP/self-01` |

Самостоятельная — это не проверочная, а изучение темы с письменным отчётом:
что поняли, какие варианты рассмотрели, чем платите за выбранный. Код,
если он по теме нужен, лежит там же в `src/`.

Курс — `OOP`, `Parallel` или `TechProg`, папка с таким именем уже есть
в вашем репозитории. Вид и номер всегда написаны в самой задаче.

Дальше в примерах — домашнее задание по ООАП, то есть `OOP/hw-01`.
Для лабораторной или самостоятельной подставьте своё.

!!! note "Что откуда брать"

    `ИМЯ-РЕПОЗИТОРИЯ` ниже — имя вашего личного репозитория, оно есть
    в комментарии робота к вашей заявке.
...
```
OOP/hw-01/src/      ← сюда код
OOP/hw-01/README.md ← сюда описание и схема (файл уже есть, заполните его)
```
...
```bash
git add OOP/hw-01
git commit -m "ООАП, ДЗ 1: модель Библиотеки"
```
...
```bash
git push -u origin OOP/hw-01
```

В ответ git напечатает ссылку вида
`https://github.com/radilov-spsu/…/pull/new/OOP/hw-01` — по ней сразу
...
- **compare:** `OOP/hw-01` — что вливаем.

Заголовок — понятный, например `ООАП, ДЗ 1: модель Библиотеки`.
...
**Сливать самому не нужно.** Кнопка **Merge pull request** у вас доступна,
но нажимать её — значит принять работу без проверки. Если уже нажали,
не переделывайте ничего: [страница про откат](accidental-merge.md).
...
| Файлы вне своей папки | Всё, что добавляете, лежит в папке работы, например `OOP/hw-01/` |
...
| Сами нажали **Merge pull request** | Работа уехала в `main` без проверки — [как вернуть](accidental-merge.md) |
